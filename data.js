...
const denID = window.location.hash.slice(1)
console.log(denID)
const denData = predpoved.find((day) => day.id === denID)
console.log(denData)
const elStranka = document.querySelector("container")


elStranka.innerHTML += `
<div class="pocasi-card">
    <h2>${denData.den}</h2>
    <p>Ranní teplota: ${denData.ranni_teplota}</p>
    <p>Odpolední teplota: ${denData.odpoledni_teplota}</p>
    <p>Večerní teplota: ${denData.vecerni_teplota}</p>
    <p>Stav: ${denData.stav_pocasi}</p>
    <p>Tlak: ${denData.tlak}</p>
    <p>Rychlost větru: ${denData.rychlost_vetru}</p>
